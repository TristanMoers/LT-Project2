...
class JShiftOp extends JBinaryExpression {

    /**
     * Construct an AST for a shift expression given its line number,
     * and the lhs and rhs operands.
     * 
     * @param line
     *            line in which the shift expression occurs in the
     *            source file.
     * @param lhs
     *            the lhs operand.
     * @param rhs
    ...
    public JShiftOp(int line, JExpression lhs, JExpression rhs) {
        super(line, ">>>", lhs, rhs);
    }
    ...
    /**
     * Generating code for the >>> operation involves generating code for the two
     * operands, and then the shift instruction.
     * 
     * @param output
     *            the code emitter (basically an abstraction for producing the
     *            .class file).
